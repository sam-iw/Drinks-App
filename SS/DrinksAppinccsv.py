...
class Round:
    ...
    def __init__(self, brewer):
        load_csv_dictionary("../persistence/preferences.csv", preferences)
        # assign a brewer
        self.brewer = brewer
        # set active to true
        self.active = True
...
def Initial_Function(number):
    if number == "1":
        ...
        #     count += 1
        save_csv_dictionary("../persistence/preferences.csv", preferences)
        # nav_options()
    elif number == "8":
        os.system("clear")
        print_options()
...
def print_list(list): # Print list function
    count = 1
    for items in list:
        print(f" {count} {items}")
        count += 1
...
# def add_preferences():
#     while True:
#         preference = input("""Add a preference in format of "person: drink" or hit enter to stop adding people: """)
#         if preference == "":
#             break
#         split_preference = preference.strip().split(": ")
#         person = split_preference[0]
    ...
    try:
        with open(filepath, "w") as dictionary_file:
            for key, value in dictionary.items():# key and values in dict two, dict_two.items()
                dictionary_file.write(f"{key}'s drink of choice is {value}" + "\n") # add to the text file, saves it (I think)
    except:
