...
class Drinks:
    ...
    def hot_drinks(self):
        try:
            ...
            print(f"\nHi {self.brewer}, your stored hot drink preference is {brewer_hot_drink[0]}, {brewer_hot_drink[1]} and {brewer_hot_drink[2]} with {brewer_hot_drink[3]} sugar(s).")
            brewer_choice = input("Are you having the usual? Y/N: ")
            if brewer_choice == "Y":
            ...
            self.hot_drink_input(self.brewer)
        while True:
            drinker = input("\nWho else would you like to include in this round? (Hit ENTER to exit) \nEnter here: ")
            if drinker == "":
                break
            else:
                try:
    ...
    def soft_drinks(self):
        try:
            ...
            print(f"\nHi {self.brewer}, your stored soft drink preference is {brewer_soft_drink}.")
            brewer_choice = input(f"Are you having the usual? Y/N: ")
            if brewer_choice == "Y":
        ...
        while True:
            drinker = input("\nWho else would you like to include in this round?: (Hit ENTER to quit)\n")
    ...
    def alcoholic_drinks(self):
        try:
            ...
            print(f"\nHi {self.brewer}, your stored alcoholic drink preference is {brewer_alcoholic}.")
            brewer_choice = input("Are you having the usual? Y/N: ")
            if brewer_choice == "Y":
                self.alcohol_round[self.brewer] = brewer_alcoholic
        ...
        while True:
            drinker = input("\nWho else would you like to include in this round?: \n(Hit ENTER to quit) Enter here: ")
            try:
                if drinker == "":  # Break out from the loop when user hits ENTER
                    break
                drinker_alcy_drink = alcoholic_drinks_pref[drinker]
                print(f"\n{drinker}'s stored alcoholic drink preference is {drinker_alcy_drink}.")
                brewer_choice = input(f"Is {drinker} having the usual? Y/N: ")
                if brewer_choice == "Y":
                    self.alcohol_round[drinker] = drinker_alcy_drink
                else:
                    alcoholic_drink_choice = input(f"\nWhat boozy drink would {drinker} like?: ")
